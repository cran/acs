...
# acs.var.2011=xmlInternalTreeParse("./data/acs_5yr_2011_var.xml")
# acs.var.2010=xmlInternalTreeParse("./data/acs_5yr_2010_var.xml")
# sf1.var.2010=xmlInternalTreeParse("./data/sf1.xml")

## online versions
## acs.var.2011=xmlInternalTreeParse("http://www.census.gov/developers/data/acs_5yr_2011_var.xml")
## acs.var.2010=xmlInternalTreeParse("http://www.census.gov/developers/data/acs_5yr_2010_var.xml")
## sf1.var.2010=xmlInternalTreeParse("http://www.census.gov/developers/data/sf1.xml")
...
# .acs.unit.levels
  ...
  api.url.maker=function(endyear, span, key, variables, geo.call) {
    ...
    api.url=paste0("http://api.census.gov/data/", endyear, "/acs", span, "?key=", key, "&get=", variables, ",NAME&", api.for, "&", api.in)
    api.url
  }
...
# original version, may cause trouble in windows?
...
setClass(Class="geo", representation =
    representation(api.for="list", api.in="list", name="character"), prototype())
  ...
  ##   geo.set.obj=new(Class="geo.set",
  ##     combine=combine,
  ##     combine.term=paste(combine.term(e1), " + ", combine.term(e2)),
  ##     geo.list=c(geo.list(e1), geo.list(e2)))
  ##   geo.set.obj
  ## })
  
  # below is attempt to fix above:
  # if geo.list(e1), geo.list(e2) are geo.sets, then take the geolists of them, recusively; should always yield flattend set.
  ...
  # NOTE: changed this recently to prevent extra nesting when only one
  ...
  geo.lookup=function(state, county, county.subdivision, place) {
  #  data(fips.state)
    state.name=NA
    if (missing(state)){return(NA)}
    ...
    if(length(state) > 1){
      ...
      a=geo.lookup(state=state[1], county=county, county.subdivision=county.subdivision, place=place)
      b=geo.lookup(state=state[2:length(state)], county=county, county.subdivision=county.subdivision, place=place)
      return(rbind.fill(a,b))}
    results=data.frame(state=state, state.name=state.name, stringsAsFactors=F)
    # check counties
  #  data(fips.county)
  #  data(fips.county.subdivision)
    ...
    if (!missing(county)) {
      ...
      # need to fix for when county is numberic vector, here and below
  ...
  geo.make=function(state, county, county.subdivision, place, tract,
    block.group, combine=F, combine.term="aggregate", check=FALSE, key="auto") {
    .geo.unit.make=function(state, county, county.subdivision,
    place, tract, block.group) {
  #    data(fips.state)
  #    data(fips.county)
  #    data(fips.county.subdivision)
      geo.obj=NA
      if(missing(state)) {
        ...
        return(NA)}
      nargs=nargs()
  # check for abbreviations/text names and convert to codes
      ...
      if (!missing(county) && is.character(county) && county !="*"){
        ...
        if (length(county)>1){
#          warning(geo.lookup(county=county, state=state))
  ...
  #      county.subdivision=fips.county.subdivision[grepl(paste("^", county.subdivision, sep=""), fips.county.subdivision$COUSUBNAME) & (fips.county.subdivision$STATEFP==state) & (!missing(county) | fips.county.subdivision$COUNTYFP==county) , 5]
        if (length(county.subdivision)>1){
#          warning(geo.lookup(county.subdivision=county.subdivision, state=state))
...
#              warning(geo.lookup(place=place, state=state))
      ...
      # sumlev 40 (state)
      if (nargs==1) geo.obj=new(Class="geo", api.for=list(state=state), api.in=list(), name=state.name)
    # sumlev 50 (state-county)
      if (nargs==2 && hasArg(county))
        geo.obj=new(Class="geo", api.for=list(county=county), api.in=list(state=state), name=paste(county.name, state.name, sep=", "))
    # sumlev 160 (state-place)
      if (nargs>2 && hasArg(place)) {
        warning("Using sumlev 160 (state-place)\n  Other levels not supported by census api at this time")
      }
      if (hasArg(place))
        geo.obj=new(Class="geo", api.for=list(place=place), api.in=list(state=state), name=paste(place.name, state.name, sep=", "))
    # sumlev 60 (state-county-county.subdivision)
      if (nargs==3 && hasArg(county) && hasArg(county.subdivision))
        geo.obj=new(Class="geo", api.for=list("county+subdivision"=county.subdivision), api.in=list(state=state, county=county), name=paste(subdivision.name, county.name, state.name, sep=", "))
    # sumlev 140 (state-county-tract)
      if (nargs==3 && hasArg(county) && hasArg(tract))
        geo.obj=new(Class="geo", api.for=list(tract=tract), api.in=list(state=state, county=county), name=paste("Tract ", tract, ", ", county.name, ", ", state.name, sep=""))
    # sumlev 150 (state-county-tract-block.group)
      if (nargs==4 && hasArg(county) && hasArg(tract) && hasArg(block.group))
        geo.obj=new(Class="geo", api.for=list("block+group"=block.group), api.in=list(state=state, county=county, tract=tract), name=paste("Tract ", tract, ", Blockgroup ", block.group, ", ", county.name, ", ", state.name, sep=""))
      if (!is.geo(geo.obj)) {
        warning("No valid geography from these arguments; returning NA\n  Perhaps add or drop levels from this request?")
      }
  #    geo.name=paste(api.for(geo.obj), names(api.for(geo.obj)), "in", paste(api.in(geo.obj), collapse=" "), sep=" ")
  #    geo.obj@name=paste(names(api.for(geo.obj)),":", api.for(geo.obj),  "in", paste(api.in(geo.obj), collapse=" "), sep="")
      geo.obj
    }
                                          # recycle!
    ...
    if (check==T){
      for (i in 1:length(geo.set.obj)){
        ...
        obj.test=acs.fetch(geography=geo.set.obj[i], key=key, variable="B01001_001",  lookup=F)
        cat("OK.\n")
      }
    }
    geo.set.obj
...
# acs.lookup
# expects table.name and/or variable to be character vectors;
# e.g., table.name=c("Age","Race") to find all matching Age and Race
# use table.number is identical to  


acs.lookup=function(endyear=2011, span=5, dataset="acs", keyword, table.name, table.number, case.sensitive=T) {
  arglist=as.list(environment())
  ...
  if (dataset=="sf1") {
        doc=xmlInternalTreeParse(system.file("extdata/sf1.xml.gz", package="acs"))
    }
  else
    if (endyear==2010) {
  ...
  if (!missing(keyword)){
    if (!case.sensitive) {str.a="contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'),'"}
    else {str.a="contains(., '"}
    str.b=paste(str.a,keyword,"')", sep="")
    str.c=paste(str.b, collapse=" and ")
    keyword=paste0("[", str.c,"]")
  } else {keyword=""}
  if (!missing(table.name)) {
    if (!case.sensitive) {str.a="contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'),'"}
    else {str.a="contains(@name, '"}
    str.b=paste(str.a,table.name,"')", sep="")
    str.c=paste(str.b, collapse=" and ")
    table.name=paste0("[",str.c, "]")
  } else {table.name=""}
  STRING=paste0("/apivariables/*",table.name,"/*",keyword)
  ...
  table.names=xpathSApply(doc, STRING, xmlGetAttr, "concept")
  ...
  if (length(names)==0){
    ...
    return(NA)}
  new(Class="acs.lookup", endyear=endyear, span=5, args=arglist, results=data.frame(variable.code=names, table.number=table.numbers, table.name=table.names, variable.name=values, stringsAsFactors=F))
}
...
acs.fetch=function(endyear=2011, span=5, geography, table.name,
    table.number, variable, keyword, key, col.names="auto", ...)
    {
      ...
      if (missing(key)){
        if (file_test("-f", system.file("extdata/key.rda", package="acs"))) {
          ...
          warning("'key' required to access Census API site for download;\n  See http://www.census.gov/developers/tos/key_request.html to request a key\n  and/or use 'key=' (or run 'api.key.install()') to avoid this error.")
          return(NA) }
      }
      endyear=as.integer(endyear)
      if (endyear < 2010 |  endyear > 2011)
        {
          warning("Current Census API only provides data for 2006-2010 and 2007-2011")
          return(NA)
        }
      if (span!=5)
        {
          warning("Current Census API only provides 5-year data;\n  Setting span=5")
          span=5
        }
      span=as.integer(span)
...
#        return(arglist)
      ...
      if (!is.acs.lookup(variable)) {
        ...
        if (variable[1]=="") {
          variables.xml=acs.lookup(keyword=keyword, table.name=table.name, endyear=endyear, dataset="acs", ...)
  ...
  #    if (identical(acs.units, "auto")) acs.units=rep("auto",(length(variables)/2))
      if (identical(col.names, "auto")) col.names=rep("auto",(length(variables)/2))
      # deal with too many variables -- API currently limits to < 50
      if (length(variables) > var.max) {
        acs.obj=cbind(acs.fetch(endyear=endyear, span=span, geography=geography, variable=c("recur", variables[1:(var.max-2)]), key=key, col.names=col.names[1:((var.max-2)/2)]), acs.fetch(endyear=endyear, span=span, geography=geography, variable=c("recur", variables[(var.max-1):length(variables)]), col.names=col.names[(1+((var.max-2)/2)):length(col.names)], key=key))
  #      acs.obj@acs.colnames=acs.clean.cols(acs.obj)
  #      acs.obj@acs.units=acs.clean.units(acs.obj)
        return(acs.obj)
      }
  ...
  # after this point, geography should be always be a single geo
      api.url=api.url.maker(endyear=endyear, span=span, key=key, variables=variables, geo.call=geography)
  ...
  # old version -- works, but problem when give table.name, etc.
  # acs.fetch=function(endyear=2011, span=5, geography, table.name="",
  #   table.number, variable="", keyword="", key, col.names="auto", ...)
  #   {
  #     var.max=40  # most var for acs call; keep even; 
  ...
  #     }
  #     if (is.acs.lookup(variable)) {
  #       if (keyword !="" | table.name != "" | !missing(table.number)) {
  #         warning("'variable' seems to already be the result of an acs.lookup\n  Ignoring keyword/table.name/table.number arguments.")}
  #       endyear=endyear(variable)
  #       span=span(variable)
  #       variables.xml=results(variable)
  #       variables=variables.xml$variable.code
  ...
  #     }
  #     if (!missing(table.number)){
  #       if (table.name !="") warning("Cannot specify both table.name and table.number; using table.number")
  #       table.name=paste(table.number, ".", sep="")}
  #     endyear=as.integer(endyear)
  #     if (endyear < 2010 |  endyear > 2011)
  #       {
  ...
  #       else {
  #         variables=paste(rep(variable, each=2), c("E","M"), sep="")}
  #       if (variable[1] !="" && (keyword[1] != "" || table.name != "" || !missing(table.number))) {warning("Cannot specify both 'variable' and 'keyword/table.name/table.number'.\n  Using 'variable' and ignoring others.")}
  #       if (keyword[1]=="" && table.name[1]=="" && !missing(table.number) && variable[1] =="") {
  #         warning("No search terms provided; returning NA")
  #         return(NA)}
  #       if (variable[1]=="") {
  #         variables.xml=acs.lookup(keyword=keyword, table.name=table.name, endyear=endyear, dataset="acs", ...)
  ...
  #                                         # pretty option to pull descriptive names from XML lookup results; only take every other, since these are the headers for estimates, not MOEs
  #     if (identical(col.names, "pretty")) {
  #       if (!is.acs.lookup(variable) && variable!="" ) {
  #         warning("\"pretty\" col.names not available when 'variable' provided.\n  Using standard variable code names for columns.")
  #         col.names="auto"}
  #       else {
  #         col.names=paste(variables.xml$table.name, variables.xml$variable.name, sep=": ")
  #       }}
...
divide.acs=function(numerator, denominator, method="ratio", verbose=T){
